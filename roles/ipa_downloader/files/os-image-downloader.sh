...
# OS Image Downloader
# =============================================================================
# Downloads Ubuntu LTS and/or Flatcar images and writes SHA256 checksum files.
# =============================================================================
...
# Flatcar image configuration (overridable via environment)
FLATCAR_IMAGE_ENABLED="${FLATCAR_IMAGE_ENABLED:-false}"
FLATCAR_IMAGE_URL="${FLATCAR_IMAGE_URL:-https://stable.release.flatcar-linux.net/amd64-usr/current/flatcar_production_qemu_image.img}"
...
set_flatcar_target_paths() {
    ...
    if [ -z "$source_filename" ]; then
        log_error "Unable to derive Flatcar filename from source URL: ${source_url}"
        return 1
    fi

    if [ -n "$FLATCAR_IMAGE_FILENAME" ] && [ "$FLATCAR_IMAGE_FILENAME" != "$source_filename" ]; then
        log_warn "Overriding Flatcar filename ${FLATCAR_IMAGE_FILENAME} with upstream filename ${source_filename}"
    fi
...
process_flatcar_image() {
    if [ -z "$FLATCAR_IMAGE_URL" ]; then
        log_error "Flatcar mirror is enabled but FLATCAR_IMAGE_URL is empty"
        return 1
    fi

    ensure_directory "$FLATCAR_IMAGE_DIR" "Flatcar image"

    if set_flatcar_target_paths "$FLATCAR_IMAGE_URL"; then
        download_url "$FLATCAR_IMAGE_URL" "$FLATCAR_IMAGE_PATH" "$FLATCAR_IMAGE_FILENAME" || true
        write_checksum "$FLATCAR_IMAGE_PATH" "$FLATCAR_IMAGE_CHECKSUM_PATH" || true
    else
        log_error "Failed to derive Flatcar target filename/path from source URL"
        return 1
    fi
}
